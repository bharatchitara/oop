...
class PassengerLogistics(Logistics):
    ...
    def validate_vehicle_type(self):
        for index in range(0,len(self.__list_vehicle)):
            if(self.__list_vehicle[index]==self.__vehicle_type):
                return index
        return -1
    
    
    def calculate_bill_amount(self):
        if(self.get_end_reading()>self.get_start_reading()):
            if(self.validate_vehicle_type() or self.validate_meter_reading()):
                self.__consumer_id=self.generate_consumer_id()
            
                distance_travelled = self.get_end_reading()-self.get_start_reading()
            
                if(self.__vehicle_type==self.__list_vehicle[0]):
                    PassengerLogistics.i=0
                elif(self.__vehicle_type==self.__list_vehicle[1]):
                    PassengerLogistics.i=1
                elif(self.__vehicle_type==self.__list_vehicle[2]):
                    PassengerLogistics.i=2
                
            
                temp= PassengerLogistics.i
            
            
                if(self.__vehicle_type== self.__list_vehicle[0]):
                    if(distance_travelled<100):
                        bill = (self.__list_charge_for_hundred[temp]*distance_travelled)
                    
                        if(bill < self.__list_minimum_charge[temp]):
                            self.__list_minimum_charge[temp] = bill
                        bill*=1.05
                    else:
                        bill = (self.__list_charge_after_hundred[temp]*(distance_travelled-100)) + (self.__list_charge_for_hundred[0]*100)
                        if(bill < self.__list_minimum_charge[temp]):
                            self.__list_minimum_charge[temp] = bill
                        bill*=1.05
        ...
        if(self.validate_carrier_type()):
            if(self.validate_meter_reading()):
                ...
                if(self.__goods_weight<=1000):
                    ...
                    charge_per_kilometer=self.__carrier_dict[self.__carrier_type]*3
                else:
                    charge_per_kilometer=200
                bill_amount=total_distance*charge_per_kilometer
                bill_amount=bill_amount+(bill_amount*1.10)+2000
                return bill_amount
            else:
                return -1
        else:
